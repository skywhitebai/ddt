...
<!--角色菜单管理-->
<div id="dlgRoleMenu" class="easyui-dialog" style="width: 500px; height: 400px; padding: 10px 20px"
     data-options="closed:true, resizable:true, modal:true, buttons:'#dlg-buttons', align:'center'">
    <input type="hidden" id="roleMenuRoleId">
    <ul id="menuTree"></ul>
    <div style="text-align:center;">
        <a href="javascript:void(0)" class="easyui-linkbutton"
           data-options="iconCls:'icon-ok'" id="btn_save_role_menu" onclick="saveRoleMenu()">确定</a>
        <a href="javascript:void(0)" class="easyui-linkbutton"
...
<script type="text/javascript">
    ...
    function showRoleMenuDialog() {
        ...
        $('#dlgRoleMenu').dialog('open').dialog('setTitle', '角色菜单管理');
        bindMenuTree();
    }
        ...
        $.post('${pageContext.request.contextPath }/roleMenu/roleMenuList', {roleId: roleId}, function (data) {
            if (data.code == '200') {
                for ( var i = 0; i <data.data.length; i++){
                    var node=$('#menuTree').tree('find', data.data[i].menuId);
                    if(node){
    ...
    function saveRoleMenu() {
        ...
        $.post('${pageContext.request.contextPath }/roleMenu/save', {roleId: roleId,menuIds:menuIds}, function (data) {
            if (data.code == '200') {
                closeDialogRoleMenu();
            }
            else {
